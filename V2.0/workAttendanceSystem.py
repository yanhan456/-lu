...
from Add_records import AddWindow
from DB import DBWindow
from Delete_record import Delete_Record
from Main import MainWindow

from Password import PasswordWindow
from QianDaoTime import WorkTimeWindow
from Modify_the_account import UserWindow
...
class WASTWO(wx.Frame):
    def __init__(self):
        wx.Frame.__init__(self,parent=None,title="员工考勤后台管理系统",size=(920,560))
    ...
    def initMenu(self):
        ...
        Ui_Search = wx.Menu()
        self.open_logcat = wx.MenuItem(Ui_Search, SEARCH, "查询")
        self.open_logcat.SetBitmap(wx.Bitmap("drawable/open_logcat.png"))
        self.open_logcat.SetFont(menu_Font)
        self.open_logcat.SetTextColour("SLATE BLUE")
        ...
        Update = wx.Menu()
        self.open_logcat = wx.MenuItem(Update,UPDATE , "修改账号")
        self.open_logcat.SetBitmap(wx.Bitmap("drawable/open_logcat.png"))
        self.open_logcat.SetFont(menu_Font)
        self.open_logcat.SetTextColour("SLATE BLUE")
        Update.Append(self.open_logcat)

        self.open_logcat = wx.MenuItem(Update, UPDATE_Password, "修改密码")
        self.open_logcat.SetBitmap(wx.Bitmap("drawable/open_logcat.png"))
        self.open_logcat.SetFont(menu_Font)
        self.open_logcat.SetTextColour("SLATE BLUE")
        Update.Append(self.open_logcat)
        ...
        Time = wx.Menu()
        self.open_logcat = wx.MenuItem(Time, WORKTIME, "修改签到时间")
        self.open_logcat.SetBitmap(wx.Bitmap("drawable/open_logcat.png"))
        self.open_logcat.SetFont(menu_Font)
        self.open_logcat.SetTextColour("SLATE BLUE")
        Time.Append(self.open_logcat)

        Delete = wx.Menu()
        self.open_logcat = wx.MenuItem(Delete, Delete_user, "删除用户")
        self.open_logcat.SetBitmap(wx.Bitmap("drawable/open_logcat.png"))
        self.open_logcat.SetFont(menu_Font)
        self.open_logcat.SetTextColour("SLATE BLUE")
        Delete.Append(self.open_logcat)

        ADDrecord = wx.Menu()
        self.open_logcat = wx.MenuItem(ADDrecord, ADD, "补签到")
        self.open_logcat.SetBitmap(wx.Bitmap("drawable/open_logcat.png"))
        self.open_logcat.SetFont(menu_Font)
        self.open_logcat.SetTextColour("SLATE BLUE")
        ADDrecord.Append(self.open_logcat)

        Deleterecord = wx.Menu()
        self.open_logcat = wx.MenuItem(Deleterecord, DELETE, "删除日志")
        self.open_logcat.SetBitmap(wx.Bitmap("drawable/open_logcat.png"))
        self.open_logcat.SetFont(menu_Font)
        self.open_logcat.SetTextColour("SLATE BLUE")
        Deleterecord.Append(self.open_logcat)
        ...
        menuBar.Append(Ui_Search, "&查询")
        menuBar.Append(Update, "&修改登录账号密码")
        menuBar.Append(Time, "&修改签到时间")
        menuBar.Append(Delete, "&删除用户")
        menuBar.Append(ADDrecord, "&补签到")
        menuBar.Append(Deleterecord, "&删除日志")
        ...
        self.Bind(wx.EVT_MENU, self.Update_username, id=UPDATE)
        self.Bind(wx.EVT_MENU, self.Update_password, id=UPDATE_Password)
        self.Bind(wx.EVT_MENU, self.Work_time, id=WORKTIME)
        self.Bind(wx.EVT_MENU, self.delete, id=Delete_user)
        self.Bind(wx.EVT_MENU, self.Add_record, id=ADD)
        self.Bind(wx.EVT_MENU, self.DeleteRecord, id=DELETE)
    ...
    def Write_Excel(self,event):
        ...
        self.infoText.AppendText("打印下班考勤日志成功（保存为/shuju_after/example.xls）\r\n")
        wb.save(r'./shuju_after/example.xls')
    ...
    def Write_Excel_Work(self,event):
        ...
        # print(self.logcat_late,"444")
        # print(str(self.logcat_id),"333")
        self.infoText.AppendText("打印上班考勤日志成功（保存为/shuju_after/Work_Attendance.xls）\r\n")
        wb.save(r'./shuju_after/Work_Attendance.xls')
