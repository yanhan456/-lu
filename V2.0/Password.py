...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.lineEdit.setGeometry(QtCore.QRect(10, 20, 201, 31))
        ...
        self.lineEdit_3.setGeometry(QtCore.QRect(10, 140, 201, 31))
        ...
        self.menubar.setGeometry(QtCore.QRect(0, 0, 234, 26))
    ...
    def Password(self):
        ...
        if user_name == str(username[0]):
            if pass_word == str(password[0]):
                update_password = self.lineEdit_3.text()
                if update_password == str(password[0]):
                    QMessageBox.information(widget, '信息提示框', '与原密码相同')
                else:
                    conn = sqlite3.connect('inspurer.db')
                    cur = conn.cursor()
                    sql = "update user set password=%s where username" % (update_password)
                    rows = cur.execute(sql)
                    conn.commit()
                    QMessageBox.information(widget, '信息提示框', '密码修改成功')
            else:
